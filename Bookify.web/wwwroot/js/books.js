$(document).ready(function () {
    ...
    datatable = $('#Books').DataTable({
        ...
        columns: [
                ...
                "render": function (data, type, row) {
                    return `<div class="symbol symbol-50px overflow-hidden me-3">
                                        <a href="/Books/Details/${row.id}">
                                                 ...
                                                 <img src="/images/${(row.image === null ? 'image-placeholder.jpg' : 'books/' + row.image)}" alt="cover" class="w-100">
                                            </div>
                                        </a>
                                    </div>
            ...
            {
                "name": "IsAvailabeForRental",
                "render": function (data, type, row) {
                    return `<span class="badge badge-light-${(row.isAvailabeForRental ? 'success' : 'warning')}">
                                        ${(row.isAvailabeForRental ? 'Availabe' : 'Not Availabe')}
                                    </span>`;
                }
            },
            {
                ...
                "render": function (data, type, row) {
                    return `<span class="badge badge-light-${(row.IsDeleted ? 'danger' : 'success')} js-status">
                                                ${(row.IsDeleted ? 'Deleted' : 'Availabe')}
                                    </span>`;
                }
            },
            {
